...
void StaplePool::add_stacks(){
    ...
    for (auto &dom : domains){
        std::cout << dom.id << " " << dom.is_seam << " " << dom.is_edge << ":\t";
        for (auto &stack : dom.stack_domains){
            std::cout << stack->id << " ";
        }
        std::cout << std::endl;
    }
...
bool formsCrossover(PDOM a, PDOM b, State_t state){
    bool result = false;
    if (state==s11) result = true;
    return result;
}
bool neitherFormCrossover(PDOM a, PDOM b, State_t sA, State_t sB){

}
void StaplePool::add_state_stacks(){
    for (auto &dom : domains){
        auto staple = dom.staple;
        for (auto &stack : dom.stack_domains){
            auto stackstaple = stack->staple;
            for (State_t state : staple->possible_states){
                if (dom.is_edge && stack->is_edge && staple->id == stackstaple->id){
                    if (!formsCrossover(domains.begin()+dom.id,stack,state))
                        dom.stackIdxs[state].push_back(stack->id);
                }
                else if (dom.is_seam && stack->is_seam && staple->id == stackstaple->id){
                    if (formsCrossover(domains.begin()+dom.id,stack,state))
                        dom.stackIdxs[state].push_back(stack->id);
                }
                else if (dom.is_seam && stack->is_seam && staple->id != stackstaple->id){
    ...
    std::cout << "id" << "\t";
    std::cout << "t" << "\t";
    for (auto state : staple_poss_state_names){
        std::cout << state << "\t";
    }
    std::cout << std::endl;
    for (auto &dom : domains){
        std::cout << dom.id << "\t";
        char type = ' ';
        if (dom.is_seam) type = 's';
        if (dom.is_edge) type = 'e';
        if (dom.is_body) type = 'b';
        std::cout << type << "\t";
        for (auto state : staple_poss_states){
            std::cout << dom.stackIdxs[state] << "\t";
        }
        std::cout << std::endl;
    }
}
