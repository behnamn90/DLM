...
void TransitionManager::initialise_reverse_transitions(){
    ...
    for (auto tr1 = transitions.begin(); tr1!=transitions.end(); ++tr1){
        for (auto tr2 = transitions.begin(); tr2!=transitions.end(); ++tr2){
            if (tr1->reverse_id == tr2->id){
                if (tr2->reverse_id != tr1->id){
                    cout << "ERRROR!\n";
                }
            }
        }
...
void TransitionManager::reset_recalculates(){
    /*
    for (auto& tr : this->transitions){
        tr.recalculate = false;
        if (ramp->T_jump.was_changed)   tr.recalculate = true;
        if (tr.crossover.second)        tr.recalculate = true;
        if (tr.type == Transition_t(invasion)){
            if (tr.affected_crossover.second) tr.recalculate = true;
        }
    }
    next->recalculate = true;
    next->reverse->recalculate = true;
    
    for (auto& tr : next->staple->transitions){
        tr->recalculate = true;
    }
    for (auto& stackdom : next->domain->stack_domains){
        for (auto& tr : stackdom->transitions){
            tr->recalculate = true;
        }
    }
    if (next->type == Transition_t(invasion)){
        for (auto& tr : next->affected_staple->transitions){
            tr->recalculate = true;
        }
        for (auto& stackdom : next->affected_domain->stack_domains){
            for (auto& tr : stackdom->transitions){
                tr->recalculate = true;
            }
        }
...
void TransitionManager::set_dG_shape(TR& tr, bool based_on_affected) {
    if (this->local){
        double E;
        if (based_on_affected) {E = cs_hack + G->total_weight(tr->affected_crossover.first);}
        else {E = cs_hack + G->total_weight(tr->crossover.first);}
        tr->dG_shape = -(gas_constant * ramp->get_T()) * constants->gamma_parameter * log(constants->C_parameter / E);
    }
    else{
        double W = 0, W2 = 0;
        ...
        for (auto loop = G->Faces.faces.begin(); loop!= G->Faces.faces.end(); ++loop){
            ...
            W+= log(constants->C_parameter / loop->weight);
        }
        for (auto loop = G->next_Faces.faces.begin(); loop!= G->next_Faces.faces.end(); ++loop){
            loop->calculate_weight(G->next_g);
            W2+= log(constants->C_parameter / loop->weight);
        }
        tr->dG_shape = -(gas_constant * ramp->get_T() * constants->gamma_parameter ) *( W2 - W );
        tr->undo_apply(G);
    }
}
 */
...
/*
         ...
         if(tr->possible){
             if (tr->recalculate){
                 ...
                 if (tr->type == normal){
                     if(tr->uni){
                         if (tr->forward){
                             set_dG_shape(tr); tr->dG += tr->dG_shape;
                             tr->rate = k_uni * exp( -tr->dG / RT );
                         }
                         else{
                             set_dG_duplex(tr); tr->dG += tr->dG_duplex;
                             set_dG_stack(tr); tr->dG += tr->dG_stack;
                             tr->rate = k_uni * exp( tr->dG / RT );
                         }
                     }
                     else{
                         if (tr->forward) {tr->rate = k_bi * tr->staple->concentration;}
                         else{
                             set_dG_duplex(tr); tr->dG += tr->dG_duplex;
                             set_dG_stack(tr); tr->dG += tr->dG_stack;
                             if (!this->local) {set_dG_shape(tr); tr->dG += tr->dG_shape;}
                             tr->rate = 1000. * k_bi * exp( tr->dG / RT );
                         }
                     }
                 }
                     ...
                     kr_in = kf * (2./(tr->b-tr->m)) * exp(dG_in/RT);
                     kr_out = kf * (2./(tr->b-tr->m)) * exp(dG_out/RT);
                     tr->k_BM = (kf*kb*kr_out) / ((kr_in*kr_out)+(kr_in*kb)+(kr_out*kb));
                     set_dG_stack(tr); tr->dG += tr->dG_stack;
                     if (this->local){
                         if (tr->final_state == s11) {set_dG_shape(tr);}
                     }
                     else{
                         set_dG_shape(tr);
                     }
                     tr->dG += tr->dG_shape;
                     if (tr->final_state == s11){
                         tr->rate = tr->k_BM * 1000 * exp( -(tr->dG/2.) / RT );
                     }
                     else{
                         tr->rate = tr->k_BM * tr->staple->concentration * exp( -(tr->dG/2.) / RT );
                     }
                 }
                 else{}
             }
 ...
 void TransitionManager::fill_rates_mixed(){ //The original from Oct19
     double k_bi = inputs->k_bi;
     double k_uni = inputs->k_uni;
     double RT = gas_constant * ramp->get_T();
     double kf, kb;
 ...
 void TransitionManager::fill_rates_metropolis(){
     double k_bi = inputs->k_bi;
     double k_uni = inputs->k_uni;
     double RT = gas_constant * ramp->get_T();
     double kf, kb;
             ...
             if (tr->recalculate){
                 ...
                 if (tr->type == normal){
                     if(tr->uni){
                         ...
                         set_dG_stack(tr); tr->dG += tr->dG_stack;
                         set_dG_shape(tr); tr->dG += tr->dG_shape;
                         if (!tr->forward) tr->dG = -1.*tr->dG;
                         if (tr->dG < 0) {tr->rate = k_uni;}
                         else {tr->rate = k_uni * exp( -tr->dG / RT );}
                     }
                     else{
                         if (tr->forward) {tr->rate = k_bi * tr->staple->concentration;}
                         else{
                     ...
                     tr->dG += tr->dG_shape;
                     if (tr->final_state == s11){
                         if (tr->dG < 0) {tr->rate = tr->staple->concentration * inputs->k_bi;}
                         else {tr->rate = 1000. * tr->k_BM * exp( -tr->dG / RT );}
                     }
                     else{
                         if (tr->dG < 0) {tr->rate = tr->staple->concentration * inputs->k_bi;}
                         else {tr->rate = tr->staple->concentration * tr->k_BM * exp( -tr->dG / RT );}
                     }
                 }
                 else{}
             }
 ...
 void TransitionManager::fill_rates_kawasaki(){
     double k_bi = inputs->k_bi;
     double k_uni = inputs->k_uni; // The thermodynamic is independent of this (iso simulation)
     double RT = gas_constant * ramp->get_T();
     double kf, kb;
     kf = inputs->k_f;
     double kr_in, kr_out, dG_in, dG_out;
             ...
             if (tr->recalculate){
                 ...
                 if (tr->type == normal){
                     if(tr->uni){
                         ...
                         set_dG_shape(tr); tr->dG += tr->dG_shape;
                         half_dG = tr->dG/2.;
                         if (tr->forward) {tr->rate = k_uni * exp( -half_dG / RT );}
                         else {tr->rate = k_uni * exp( half_dG / RT );}
                     }
                     else{
                         if (tr->forward) {tr->rate = k_bi * tr->staple->concentration;}
                         else{
